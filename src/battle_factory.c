...
static const u16 factoryMonsGen9[] = {
    ...
    1294, 1295, 1296, 1321, 1322, 1323, 1324, 1325, 1326, 1351, 1352, 1353, 1354, 1376, 1377, 1378, 1379, 1382, 1383, 1406, 1407, 1408, 1413, 1414, 1432,
...
static void GenerateInitialRentalMons(void)
{
    ...
    if (hasTatsugiri != hasDondozo)
    {
        ...
        if (hasTatsugiri)
        {
            requiredSpecies = SPECIES_DONDOZO;
            requiredMonId = (Random() % 2 == 0) ? FRONTIER_MON_DONDOZO_1 : FRONTIER_MON_DONDOZO_2; // 随机选择Dondozo的ID
        }
        else
        {
            requiredSpecies = SPECIES_TATSUGIRI_CURLY;
            requiredMonId = (Random() % 2 == 0) ? FRONTIER_MON_TATSUGIRI_CURLY_1  : FRONTIER_MON_TATSUGIRI_CURLY_2; // 随机选择Tatsugiri的ID
        }

        // 步骤3：随机替换一个非目标物种的位置
        if (replaceCount > 0)
        {
            u8 slot = replaceSlots[Random() % replaceCount]; // 随机选择槽位
